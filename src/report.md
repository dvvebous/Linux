...
![фывфыв](./Screenshoots/Part_1.png)
...
1. Выполнил команду для смены имени машины: sudo vi /etc/hostname - открыл в текстом редакторе vi с правами суперпользователя файл, хранящий название машины и заменил прежнее на aemmazub-1
...
Не влезло в терминал, изменил hostname с aemmazub-1 на aemmazub-1234, проверил что находился под нужным пользователем
...
Открыл файл text_vim.txt через команду: vim text_vim.txt

Закрыл с сохранениями: ESC + : + w + q

![a](./Screenshoots/Part_7_vim_2.png)

Открыл файл text_vim.txt через команду: vim text_vim.txt

Прописал \:s/aemmazub/21 School 21, нажал Enter

Прописал для выхода без сохранения: :q!
...
Команда для замены :s/old_word/new_word
Команда для поиска /searded_word

![a](./Screenshoots/Part_7_nano_1.png)

Открыл через nano test_nano.txt
ctrl+x, y, enter
...
Стер aemmazub, написал 21 School 21
ctrl+x, n
...
proto - протокол, по которому осуществляется соедение

Recv-Q - количество байтов которые должны были полученны приложением, но помещены в буфер приема пакетов tcp/ip

Send-Q - количество байтов не отправленных или не подтверженных байтов, которые помещены в буфер отправки tcp/ip

Local address - локальный адрес и порт

Foreigh address - IP-адрес и номер порта удаленного компьютера, к которому подключен сокет.

State - состояние tcp-соединения

0.0.0.0 - адрес, который позволяет получить службе все доступные ip-адреса доступные на компьютере 
